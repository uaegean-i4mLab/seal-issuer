...
class Interaction extends transports_1.Transportable {
    ...
    async processInteractionToken(token) {
        // verify
        try {
            await this.ctx.ctx.idw.validateJWT(token, this.messages.length ? this.messages[this.messages.length - 1] : undefined, this.ctx.ctx.resolver);
        }
        catch (err) {
    ...
    async storeCredentialMetadata() {
        ...
        try {
            ...
            return Promise.all(metadatas.map(metadata => this.ctx.ctx.credentials.storeCredentialType(metadata)));
        }
        catch (err) {
